...
async function redisCommand(command) {
  ...
  if (
    ...
    throw new Error(
      body?.error ||
      `Persistent storage request failed (${response.status})`
    );
  }
...
async function loadStored(
  ...
  if (PERSISTENT_STORAGE) {
    try {
      ...
      console.error(
        `Persistent storage read failed for ${key}:`,
        error?.message || error
      );
    }
  }
...
async function load() {
  ...
  try {
    const remote = await persistentGet(
      'sggs-tnp:content'
    );
    ...
    console.error(
      'Persistent content initialization failed:',
      error?.message || error
    );
  }
...
async function save(data) {
  ...
  const run = contentWriteLock.then(
    async () => {
      if (PERSISTENT_STORAGE) {
        await persistentSet(
          'sggs-tnp:content',
...
async function ensureAdmin() {
  if (process.env.NODE_ENV === 'production') {
    if (!DEAN_EMAIL || !DEAN_PASSWORD) {
      return null;
    }

    if (
      DEAN_PASSWORD.length <
      PASSWORD_MIN
    ) {
      throw new Error(
        `DEAN_PASSWORD must be at least ${PASSWORD_MIN} characters.`
      );
    }
  ...
  if (
    ...
    throw new Error(
      `DEAN_PASSWORD must be at least ${PASSWORD_MIN} characters for first-time setup.`
    );
  }
...
function publicSnapshot(data) {
  ...
  return {
    ...
    settings: {
      instituteName:
        safeSettings.instituteName,

      shortName:
        safeSettings.shortName,
      ...
      tnpEmail:
        safeSettings.tnpEmail,

      logo:
        safeSettings.logo,

      tnpLogo:
        safeSettings.tnpLogo,

      favicon:
        safeSettings.favicon,

      footerText:
        safeSettings.footerText,

      heroVideo:
        safeSettings.heroVideo,
...
function sanitizePatch(
  ...
  if (unknown.length) {
    throw new BadRequestError(
      `Unsupported content field: ${unknown[0]}`
    );
  }
  ...
  if (
    'departmentHistory' in
    body &&
    (
      typeof body.departmentHistory !==
        'object' ||
      Array.isArray(
    ...
    throw new BadRequestError(
      'Invalid departmentHistory payload'
    );
  }
  ...
  for (
    ...
    if (
      ...
      throw new BadRequestError(
        'Invalid URL/media reference in content payload'
      );
    }
  }
  ...
  for (
    ...
    if (
      ...
      throw new BadRequestError(
        'Invalid placement photo reference'
      );
    }
  }
    ...
    if (
      ...
      if (!admin) {
        return json(
          ...
          {
            message:
              'Dean authentication is not configured. Set DEAN_EMAIL and a strong DEAN_PASSWORD on the server and restart once.'
          }
        );
      }
      ...
      res.setHeader(
        ...
        `${SESSION_COOKIE}=${encodeURIComponent(
          token
        )}; HttpOnly; SameSite=${sameSite}; Path=/; Max-Age=${
          SESSION_TTL_MS / 1000
        };${secure}`
      );
    ...
    if (
      ...
      if (
        ...
        return json(
          ...
          {
            message:
              'Admin write rate limit reached.'
          }
        );
      }
    ...
    if (
      ...
      if (
        ...
        return json(
          ...
          {
            message:
              'Career Copilot is not configured on the server.'
          }
        );
      }
      ...
      if (
        ...
        return json(
          ...
          {
            message:
              'Career Copilot rate limit reached.'
          }
        );
      }
      ...
      const snapshot =
        JSON.stringify({
          ...
          departmentHistory:
            published.departmentHistory,

          funnel:
            published.funnel,

          announcements:
            published.announcements.slice(
              0,
              20
            ),

          recruiters:
            published.recruiters.slice(
              0,
              80
            ),

          documents:
            published.documents.slice(
              0,
              40
            ),

          calendar:
            published.calendar.slice(
              0,
              40
            ),

          placementProcess:
            published.placementProcess,

          policyRules:
            published.policyRules,

          recruiterBenefits:
            published.recruiterBenefits,
      ...
      const system =
        `You are the official SGGSIE&T Training & Placement Cell website assistant.
...
Published CMS snapshot: ${snapshot}`;
      ...
      let answer =
        'The requested SGGSIE&T T&P information is not currently published.';
...
if (
  ...
  server.listen(
    ...
    () =>
      console.log(
        `SGGS T&P API running on http://localhost:${PORT}`
      )
  );
}